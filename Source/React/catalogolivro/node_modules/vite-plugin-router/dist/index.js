...
function getFilesWithExtensions(dir) {
  ...
  function traverseDirectory(dir2, currentPath = "") {
    const items = import_fs.default.readdirSync(dir2);
    items.forEach((item) => {
      const itemPath = import_path.default.join(dir2, item);
      const isDirectory = import_fs.default.statSync(itemPath).isDirectory();
      if (isDirectory) {
        const newPath = import_path.default.join(currentPath, item).replace(/\\/g, ROOT_DIR);
        traverseDirectory(itemPath, newPath);
        return;
      }
      const fileExtension = import_path.default.extname(item);
...
function generateRoutes2({ dir = "", outDir = "" }) {
  ...
  const filePath = import_path2.default.join(CURRENT_DIR, outDir, fileName);
  import_fs2.default.writeFileSync(filePath, generateTemplate, "utf-8");
...
function viteRouter({
  ...
  const dirPath = import_path3.default.resolve(CURRENT_DIR, dir);
  const chokidarWatcher = import_chokidar.default.watch(dirPath);
  const debounceGenerateRoutes = import_lodash.default.debounce(generateRoutes2, 100);
  return {
